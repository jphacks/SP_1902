...
1. スライド操作
   - スライドを進める：《＋ボタン》を2回押す
   - スライドを戻る：《−ボタン》を2回押す
   - 方向を指定してアニメーションする：「始点」と「終点」それぞれで《＋ボタン》を1回ずつ押す（例：左から右にフェードアウトを書ける場合，モーションのスタート位置で《＋ボタン》を一回押し，左側にモーションしてから《＋ボタン》をもう一度押す）
1. その他
   - 画面内の《＜》を押すと方向のないフェードアウトのアニメーションで次のスライドに遷移
   - 画面内の《＞》を押すと前のスライドに遷移
   - 指定したい方向に対して端末を水平に持ってモーションすると，自分の思った方向に遷移しやすいです．（横方向にモーション：スマホ画面が左に向いており，ボリュームボタンが上にある．縦方向：スマホ画面が下に向いており，ボリュームボタンが左がにある．）
